...
class ChatApiService {
  ...
  async getOrCreateChatSession(otherUserPhone: string): Promise<ChatSession> {
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions?otherUserPhone=${encodeURIComponent(otherUserPhone)}`, {
      method: 'POST',
  ...
  async getChatSessions(page: number = 0, size: number = 20): Promise<{
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/chat/sessions?page=${page}&size=${size}`,
      {
        headers: await this.getAuthHeaders(),
      }
    );
  ...
  async getAllChatSessions(): Promise<ChatSession[]> {
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions/all`, {
      headers: await this.getAuthHeaders(),
    });
    return this.handleResponse<ChatSession[]>(response);
  }
  ...
  async sendMessage(chatSessionId: number, messageContent: string): Promise<ChatMessage> {
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions/${chatSessionId}/messages`, {
      method: 'POST',
      headers: await this.getAuthHeaders(),
  ...
  async getMessages(chatSessionId: number, page: number = 0, size: number = 50): Promise<{
    ...
    const response = await fetch(
      `${API_BASE_URL}/api/chat/sessions/${chatSessionId}/messages?page=${page}&size=${size}`,
      {
        headers: await this.getAuthHeaders(),
      }
    );
  ...
  async getAllMessages(chatSessionId: number): Promise<ChatMessage[]> {
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions/${chatSessionId}/messages/all`, {
      headers: await this.getAuthHeaders(),
    });
    return this.handleResponse<ChatMessage[]>(response);
  }
  ...
  async markMessagesAsRead(chatSessionId: number): Promise<void> {
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions/${chatSessionId}/read`, {
      method: 'POST',
      headers: await this.getAuthHeaders(),
    });
    if (!response.ok) {
  ...
  async getOtherParticipant(chatSessionId: number): Promise<{
    ...
    const response = await fetch(`${API_BASE_URL}/api/chat/sessions/${chatSessionId}/other-participant`, {
      headers: await this.getAuthHeaders(),
    });
    return this.handleResponse(response);
  }
