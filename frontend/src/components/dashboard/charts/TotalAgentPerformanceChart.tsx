...
interface TotalAgentPerformanceData {
  trendsByAgent: Record<string, TrendDataPoint[]>;
  agents: string[];
}

interface TotalAgentPerformanceChartProps {
  data: TotalAgentPerformanceData;
  title?: string;
  className?: string;
  loading?: boolean;
  error?: string | null;
...
export default function TotalAgentPerformanceChart({
  data,
  title,
  className = "",
  loading = false,
...
}: TotalAgentPerformanceChartProps) {
  ...
  const chartTitle = title || t('dashboard.charts.totalAgentPerformance');
  ...
  const chartData = {
    ...
    datasets: filteredAgents.map((agent) => {
      ...
      // Forward fill: carry forward the last known cumulative total
      let lastKnownTotal = 0;
      const dataWithForwardFill = allDates.map(date => {
        const dataPoint = agentData.find(point => point.date === date);
        if (dataPoint) {
          lastKnownTotal = dataPoint.totalCustomers;
        }
        return lastKnownTotal; // Use last known total if no data for this date
      });

      return {
        label: agent,
        data: dataWithForwardFill,
        borderColor: colors!.border,
        backgroundColor: colors!.background,
        tension: 0, // Use 0 for stepped appearance, or 0.4 for smooth curves
        pointRadius: 4,
        pointHoverRadius: 6,
        pointBorderWidth: 2,
        ...
        fill: false,
        hidden: false,
        spanGaps: true, // Allow Chart.js to connect points across null values
      };
    }),
  };
          ...
          beforeBody: function(tooltipItems) {
            ...
            return sortedItems.map(item => {
              ...
              if (typeof value === 'number') {
                return `${label}: ${value.toLocaleString()} ${t('dashboard.charts.totalCustomers')}`;
              }
              return `${label}: N/A`;
            }).join('\n');
          },
