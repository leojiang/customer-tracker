...
class CustomerControllerTest {
    ...
    void shouldGetCustomersWithPaginationSuccessfully() throws Exception {
        ...
        when(customerService.searchCustomers(any(), any(), any(), any(), any(), anyBoolean(), any(Pageable.class)))
        ...
        verify(customerService).searchCustomers(eq(null), eq(null), eq(null), eq(null), any(), eq(false), any(Pageable.class));
    }
    ...
    void shouldGetCustomersWithSearchParameters() throws Exception {
        ...
        when(customerService.searchCustomers(eq("john"), eq("123"), eq(CustomerStatus.CUSTOMER_CALLED), 
                eq("test"), any(), eq(false), any(Pageable.class)))
            .thenReturn(customerPage);
        ...
        verify(customerService).searchCustomers(eq("john"), eq("123"), eq(CustomerStatus.CUSTOMER_CALLED), 
            eq("test"), any(), eq(false), any(Pageable.class));
    }
    ...
    void shouldUpdateCustomerSuccessfully() throws Exception {
        ...
        // When & Then
        mockMvc.perform(patch("/api/customers/{id}", testCustomerId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void shouldRestoreCustomerSuccessfully() throws Exception {
        ...
        // When & Then
        mockMvc.perform(post("/api/customers/{id}/restore", testCustomerId))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
            .andExpect(jsonPath("$.name").value("John Doe"));
    ...
    void shouldTransitionStatusSuccessfully() throws Exception {
        // Given
        CustomerController.StatusTransitionRequest request = new CustomerController.StatusTransitionRequest();
        ...
        // When & Then
        mockMvc.perform(post("/api/customers/{id}/status-transition", testCustomerId)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request)))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
        ...
        verify(customerService).transitionStatus(testCustomerId, CustomerStatus.REPLIED_TO_CUSTOMER, 
            "Customer responded positively");
    }
    ...
    void shouldReturn400ForInvalidStatusTransition() throws Exception {
        ...
        verify(customerService).transitionStatus(testCustomerId, CustomerStatus.BUSINESS_DONE, "Invalid transition");
    }
    ...
    void shouldGetStatusHistorySuccessfully() throws Exception {
        // Given
        List<StatusHistory> statusHistory = Arrays.asList(
            createStatusHistory(testCustomer, null, CustomerStatus.CUSTOMER_CALLED, "Initial"),
            createStatusHistory(testCustomer, CustomerStatus.CUSTOMER_CALLED, CustomerStatus.REPLIED_TO_CUSTOMER, "Responded")
        );
        ...
        // When & Then
        mockMvc.perform(get("/api/customers/{id}/status-history", testCustomerId))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void shouldValidateTransitionSuccessfully() throws Exception {
        ...
        // When & Then
        mockMvc.perform(get("/api/customers/{id}/can-transition-to/{status}", 
                testCustomerId, "REPLIED_TO_CUSTOMER"))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON))
    ...
    void shouldHandlePaginationParameterValidation() throws Exception {
        ...
        mockMvc.perform(get("/api/customers")
                .param("page", "-1")
                .param("limit", "200")) // Above max limit
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.page").value(1))
            .andExpect(jsonPath("$.limit").value(100)); // Should be capped at 100

        // Verify pagination was corrected
        verify(customerService).searchCustomers(any(), any(), any(), any(), any(), eq(false), 
            argThat(pageable -> pageable.getPageNumber() == 0 && pageable.getPageSize() == 100));
    }

    private StatusHistory createStatusHistory(Customer customer, CustomerStatus fromStatus, 
