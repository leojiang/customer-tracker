...
-- Step 1: Drop the old valid_role check constraint
ALTER TABLE sales DROP CONSTRAINT IF EXISTS valid_role;

-- Step 2: Update existing SALES role to OFFICER
UPDATE sales SET role = 'OFFICER' WHERE role = 'SALES';

-- Step 3: Recreate the valid_role check constraint with new roles
ALTER TABLE sales ADD CONSTRAINT valid_role
  CHECK (role IN ('ADMIN', 'OFFICER', 'CUSTOMER_AGENT'));

-- Step 4: Create customer_delete_requests table
...
CREATE INDEX IF NOT EXISTS idx_delete_requests_status ON customer_delete_requests(request_status);
CREATE INDEX IF NOT EXISTS idx_delete_requests_customer ON customer_delete_requests(customer_id);
CREATE INDEX IF NOT EXISTS idx_delete_requests_requested_by ON customer_delete_requests(requested_by);
CREATE INDEX IF NOT EXISTS idx_delete_requests_created_at ON customer_delete_requests(created_at DESC);
