...
public interface StatusHistoryRepository extends JpaRepository<StatusHistory, UUID> {
     ...
     */
    @Query("SELECT sh FROM StatusHistory sh WHERE sh.customer.id = :customerId ORDER BY sh.changedAt DESC")
     ...
     */
    @Query("SELECT sh FROM StatusHistory sh WHERE sh.customer.id = :customerId ORDER BY sh.changedAt DESC")
    Page<StatusHistory> findByCustomerIdOrderByChangedAtDesc(@Param("customerId") UUID customerId, Pageable pageable);

    /**
     * Find latest status change for a customer
     */
    @Query("SELECT sh FROM StatusHistory sh WHERE sh.customer.id = :customerId ORDER BY sh.changedAt DESC LIMIT 1")
     ...
     */
    @Query("SELECT sh FROM StatusHistory sh WHERE sh.fromStatus = :fromStatus AND sh.toStatus = :toStatus ORDER BY sh.changedAt DESC")
    List<StatusHistory> findByFromStatusAndToStatusOrderByChangedAtDesc(
        @Param("fromStatus") CustomerStatus fromStatus,
        @Param("toStatus") CustomerStatus toStatus
    );

    /**
     * Delete all status history for a customer (cascade should handle this, but explicit method for testing)
     */
