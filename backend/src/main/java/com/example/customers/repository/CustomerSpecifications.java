...
public class CustomerSpecifications {
  ...
  public static Specification<Customer> hasNameContaining(String name) {
    return (root, query, criteriaBuilder) -> {
      ...
      return criteriaBuilder.like(
          criteriaBuilder.lower(root.get("name")), "%" + name.toLowerCase().trim() + "%");
    };
  }
  ...
  public static Specification<Customer> hasCompanyContaining(String company) {
    return (root, query, criteriaBuilder) -> {
      ...
      return criteriaBuilder.like(
          criteriaBuilder.lower(root.get("company")), "%" + company.toLowerCase().trim() + "%");
    };
  }
    ...
    return (root, query, criteriaBuilder) -> {
      ...
      if (searchQuery != null && !searchQuery.trim().isEmpty()) {
        ...
        searchPredicates.add(
            criteriaBuilder.like(criteriaBuilder.lower(root.get("name")), lowerQuery));

        // Search in phone
        searchPredicates.add(
            criteriaBuilder.like(root.get("phone"), "%" + searchQuery.trim() + "%"));

        // Search in company
        searchPredicates.add(
            criteriaBuilder.like(criteriaBuilder.lower(root.get("company")), lowerQuery));
