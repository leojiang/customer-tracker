...
@Tag(name = "Customer Management", description = "APIs for managing customers, status transitions, and history tracking")
@RestController
...
public class CustomerController {
    ...
    @Operation(
        ...
        description = "Retrieve a paginated list of customers with optional search filters by name, phone, company, or status"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<CustomerPageResponse> getCustomers(
        ...
        // Get current user's sales phone for filtering (non-admin users can only see their own customers)
        String filterBySalesPhone = getCurrentUserSalesPhone();
        
        Page<Customer> customers = customerService.searchCustomers(q, phone, status, company, filterBySalesPhone, includeDeleted, pageable);
    ...
    @Operation(
        ...
        description = "Create a new customer with the provided information. Phone number must be unique."
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "201", description = "Customer created successfully",
                content = @Content(schema = @Schema(implementation = Customer.class))),
        @ApiResponse(responseCode = "400", description = "Invalid input data or duplicate phone number")
    })
    @PostMapping
    ...
    @Operation(
        ...
        description = "Change the status of a customer and record the transition in history with an optional reason"
    )
    @ApiResponses(value = {
    ...
    public ResponseEntity<Customer> transitionStatus(
            ...
            @Parameter(description = "Status transition details", required = true)
        ...
        try {
            ...
            Customer updatedCustomer = customerService.transitionStatus(id, request.getToStatus(), request.getReason());
            return ResponseEntity.ok(updatedCustomer);
        } catch (EntityNotFoundException e) {
            return ResponseEntity.notFound().build();
        } catch (IllegalArgumentException e) {
     ...
     * Get status history for customer
     */
    @GetMapping("/{id}/status-history")
    public ResponseEntity<List<StatusHistory>> getStatusHistory(@PathVariable UUID id,
                                                               @RequestParam(defaultValue = "1") int page,
    ...
    public ResponseEntity<CustomerService.CustomerStatistics> getStatistics(
            @RequestParam(defaultValue = "false") boolean includeDeleted) {
        CustomerService.CustomerStatistics stats = customerService.getCustomerStatistics(includeDeleted);
    ...
    @Operation(
        ...
        description = "Get all valid status transitions for a specific customer based on current status and business rules"
    )
    @ApiResponses(value = {
        @ApiResponse(responseCode = "200", description = "Valid transitions retrieved successfully"),
        @ApiResponse(responseCode = "404", description = "Customer not found")
    })
    ...
    public static class CreateCustomerRequest {
        ...
        public String getBusinessRequirements() { return businessRequirements; }
        public void setBusinessRequirements(String businessRequirements) { this.businessRequirements = businessRequirements; }
        ...
        public CustomerStatus getCurrentStatus() { return currentStatus; }
        public void setCurrentStatus(CustomerStatus currentStatus) { this.currentStatus = currentStatus; }
    }
    ...
    public static class CustomerPageResponse {
        ...
        public CustomerPageResponse(List<Customer> items, long total, int page, int limit, int totalPages) {
            this.items = items;
            this.total = total;
            this.page = page;
            this.limit = limit;
